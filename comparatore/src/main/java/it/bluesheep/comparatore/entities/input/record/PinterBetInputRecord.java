...
public class PinterBetInputRecord extends AbstractScrapingInputRecord implements IKeyEventoComparator{

	private static Logger logger = Logger.getLogger(PinterBetInputRecord.class);
	
	public PinterBetInputRecord(Date dataOraEvento, Sport sport, String campionato, String partecipante1,
			String partecipante2, String filler) {
		super(dataOraEvento, sport, campionato, partecipante1, partecipante2, filler);
		this.source = Service.PINTERBET_SERVICENAME;
		this.liquidita = -1D;
		this.bookmakerName = "Pinterbet";
	}
	
	public PinterBetInputRecord(AbstractInputRecord record) {
		super(record);
		this.source = Service.PINTERBET_SERVICENAME;
		this.liquidita = -1D;
		this.bookmakerName = "Pinterbet";
