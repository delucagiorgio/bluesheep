...
public class GoldBetInputRecord extends AbstractScrapingInputRecord implements IKeyEventoComparator{

	private static Logger logger = Logger.getLogger(GoldBetInputRecord.class);
	
	public GoldBetInputRecord(Date dataOraEvento, Sport sport, String campionato, String partecipante1,
			String partecipante2, String filler) {
		super(dataOraEvento, sport, campionato, partecipante1, partecipante2, filler);
		this.source = Service.GOLDBET_SERVICENAME;
		this.liquidita = -1D;
		this.bookmakerName = "Goldbet";
	}
	
	public GoldBetInputRecord(AbstractInputRecord record) {
		super(record);
		this.source = Service.GOLDBET_SERVICENAME;
		this.liquidita = -1D;
		this.bookmakerName = "Goldbet";
